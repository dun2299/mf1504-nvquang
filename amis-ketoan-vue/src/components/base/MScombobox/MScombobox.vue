<template>
  <div class="combobox" :class="{ 'input-err': isErr }">
    <input type="text" class="input combobox__input" v-model="textInput" @input="inputOnChange" @blur="inputOnBlur"
      @keydown="selectItemUpDown" />
    <button class="button combobox__button" @click="btnSelectDataOnClick" @keydown="selectItemUpDown" tabindex="-1">
      <!-- <i class="fa-solid fa-chevron-down"></i> -->
      <img :src="require('./icon/down.png')" alt="" srcset="" width="24" height="24" />
    </button>
    <div v-if="isShowListData" class="combobox__data" ref="combobox__data" v-clickoutside="hideListData">
      <a class="combobox__item" v-for="(item, index) in dataFilter" :class="{
        ...
        @click="itemOnSelect(item, index)" @focus="saveItemFocus(index)" @keydown="selectItemUpDown(index)"
        @keyup="selectItemUpDown(index)" tabindex="1">
        <div class="combobox__item-icon">
          <img v-show="index == indexItemSelected" :src="require('./icon/check.png')" alt="" srcset="" width="14"
            height="11" />
        </div>
